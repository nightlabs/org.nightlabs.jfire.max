 ...
 *
 * @jdo.fetch-group name="PFMappingAccountantDelegate.moneyFlowMappings" fields="moneyFlowMappings"
 * @jdo.fetch-group name="PFMappingAccountantDelegate.this" fields="moneyFlowMappings"
 */
				...
				for (Iterator iter = productType.getNestedProductTypes().iterator(); iter.hasNext();) {
					...
					nested.add(
							new ProductTypeProvider(
									PFMappingAccountantDelegate.getPackageType(nestedType),
				...
				for (Iterator iter = articlePrice.getNestedArticlePrices().iterator(); iter.hasNext();) {
					...
					nested.add(
							new ArticlePriceTypeProvider(
									PFMappingAccountantDelegate.getPackageType(nestedPrice),
									nestedPrice
								)
					);
				}
...
//		Map<ResolvedMapKey, ResolvedMapEntry> result = new HashMap<ResolvedMapKey, ResolvedMapEntry>();
	...
	public void resolveProductTypeMappings(ResolveProductTypeProvider productTypeProvider, String packageType, Map<ResolvedMapKey, ResolvedMapEntry> resolvedMappings, int delegationLevel) {
		...
		if (entry != null) {
			...
			LocalAccountantDelegate _delegate = productTypeProvider.getProductType().getLocalAccountantDelegate();
	...
	protected ResolvedMapEntry addProductTypeMappings(ProductType productType, String packageType, int delegationLevel) {
		...
		while (!delegateHierarchy.isEmpty()) {
			LocalAccountantDelegate _delegate = (LocalAccountantDelegate) delegateHierarchy.removeLast();
			if (!(_delegate instanceof PFMappingAccountantDelegate)) {
				throw new IllegalStateException("There is an inconsistencey in the hierarchy of the LocalAccountantDelegate: " + _delegate.getLocalAccountantDelegateID() + " one of its nodes is not an " + PFMappingAccountantDelegate.class.getSimpleName());
			}
			PFMappingAccountantDelegate delegate = (PFMappingAccountantDelegate) _delegate;
	...
	public void bookArticle(OrganisationLegalEntity mandator, User user,
		...
		PriceFragmentType totalType = PriceFragmentType.getTotalPriceFragmentType(pm);
	...
	public void bookProductTypeParts(OrganisationLegalEntity mandator, User user, LinkedList<ArticlePrice> articlePriceStack, int delegationLevel, BookMoneyTransfer container, Set<Anchor> involvedAnchors) {
		...
		Map<Anchor, Map<Anchor, Collection<BookInvoiceTransfer>>> bookInvoiceTransfers = new HashMap<Anchor, Map<Anchor, Collection<BookInvoiceTransfer>>>();
		bookProductTypeParts(mandator, user, resolvedMappings, articlePriceStack, bookInvoiceTransfers, delegationLevel, container, involvedAnchors);
	}
		...
		)
	{
		...
		if (delegate instanceof PFMappingAccountantDelegate) {
			...
			((PFMappingAccountantDelegate) delegate).bookProductTypeParts(
					mandator,
					user, 
					resolvedMappings,
					articlePriceStack,
...
// 		boolean topLevel = articlePrice.getPackageArticlePrice() == null;
		...
		{
			delegateBookProductTypeParts(
					mandator, user, resolvedMappings, productType, 
					articlePriceStack, bookInvoiceTransfers, delegationLevel, container, involvedAnchors
			);
		}
		...
		) 
	{
		...
		if (haveToDelegateBooking(resolvedMappings, productType, packageType, delegationLevel)) {
			delegateBookProductTypeParts(mandator, user, resolvedMappings, productType, articlePriceStack, bookInvoiceTransfers, delegationLevel, container, involvedAnchors);
			return;
		}
...
//	 recurse
	 ...
	 * @param bookArticle TODO
	 */
	private void makeDimensionNodes(DimensionNode parent, List<String> dimensionIDs, int idx, Article bookArticle, ProductType productType) {
		String moneyFlowDimensionID = (String)dimensionIDs.get(idx);
		String[] dimValues = (String[])CollectionUtil.collection2TypedArray(getDimensionValues(moneyFlowDimensionID, bookArticle, productType), String.class);
	 ...
	 * @param bookArticle TODO
	 */
	public List<DimensionNode> getDimensionLeafNodes(Article bookArticle, ProductType productType) {
	 ...
	 * @param bookArticle TODO
	 */
	protected List getDimensionValues(String moneyFlowDimensionID, Article bookArticle, ProductType productType) {
	...
	/**
	 * Returns the mapping key for the given combination of 
	...
	{
		String mappingKey = MoneyFlowMapping.getMappingKey(ProductType.getPrimaryKey(productTypeID.organisationID, productTypeID.productTypeID), packageType, currencyID);
		for (String dimensionID : getMoneyFlowDimensionIDs()) {
			mappingKey = mappingKey + "/"  + ((String) dimensionValues.get(dimensionID));
		}
		return mappingKey;
	}
	...
	public static List<String> DIMENSION_IDS = CollectionUtil.array2ArrayList(
			new String[] {
	...
	/**
	 * Returns a List of {@link BookInvoiceTransfer} with transfers
		...
		) 
	{
		...
		if (priceFragmentType.getPriceFragmentTypeID().equals(PriceFragmentType.PRICE_FRAGMENT_TYPE_ID_TOTAL.priceFragmentTypeID)) 
	...
	/**
	 * Prepares the articlePrice so it (and all its nested articlePrices) has a PriceFragment defined that
	 * has the amount that is missing so that all Fragments with containerFragmentType as
	 * container together with the new fragment have the amount defined for containerFragmentType.
	 */
	protected void prepareArticlePrice(PriceFragmentType containerFragmentType, PriceFragmentType restPriceFragmentType, ArticlePrice articlePrice) {
		...
		for (Iterator iter = articlePrice.getFragments().iterator(); iter.hasNext();) {
			...
			if (fragment.getPriceFragmentType().getContainerPriceFragmentType() == null)
				continue;
			if (fragment.getPriceFragmentType().getContainerPriceFragmentType().getPrimaryKey().equals(totalPK)) {
		...
		) 
	{
		...
		for (PriceFragmentType searchPriceFragmentType : searchPriceFragmentTypes) {
			...
			if (packagingUpperMapping != null) {
				...
				return
						new PFMappingAccountantDelegate.BookInvoiceTransfer(
								from,
								to,
								amount
							); 
