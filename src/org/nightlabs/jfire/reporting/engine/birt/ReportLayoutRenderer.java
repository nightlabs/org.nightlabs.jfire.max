...
public interface ReportLayoutRenderer {
	 ...
	 *
	 * @param pm 
	 * 			A {@link PersistenceManager} that might be used to obtain information from the datastore.
	 * @param renderRequest 
	 * 			The request for rendering, holding the layout id to be rendered, the format it should be rendered to and the report parameters.
	 * 			Note that the parameters passed here were already parsed for BIRT. These should be passed to the task.
	 * @param task 
	 * 			The task that will be run to render the layout.
	 ...
	 * @param parsedParams 
	 * 			The map of ready parsed parameters. This should be passed to the task.
	 * @param fileName 
	 * 			The name that should be used as entry-file-name for the report. 
	 * 			(Without file-extension. This will be the output-formats name).
	 * @param layoutRoot 
	 * 			The folder that should be used to create the report in.
	 * @param prepareResultForTransfer 
	 * 			Whether or not to set the result's data {@link RenderedReportLayout#getData())} 
	 * 			to the appropriate content. See {@link ReportLayoutRendererUtil#prepareRenderedLayoutForTransfer(File, RenderedReportLayout, String, boolean)}
	 *
